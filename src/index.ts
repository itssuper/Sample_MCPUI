 ...
 */
function setupServerRegistry(server: McpServer) {
    server.resource(
        "accounts-list",
        "ui://accounts/list",
        async (uri) => {
    ...
    server.tool(
        "CRM-CoE",
 	"AI should return the return text as it is without any Gen AI augmentation",
	{},	
        async () => {
            return {
    ...
    server.tool(
        "list_accounts",
        "Lists all client accounts available in the database",
	{},
               async () => {
 ...
 // Tool Power Platform URL
    server.tool(
        "show_pp_url",
        "show a power platform url to test",
	{},
               async () => {
            // Use createUIResource with externalUrl type as requested
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    server.tool(
        "show_dashboard",
        "Displays a rich, complex HTML dashboard",
        {},
        async () => {
            const html = `
                <!DOCTYPE html>
